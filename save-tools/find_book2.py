...
# target skills -> want book id
targets=[b'|4|70069|',b'|4|60036|',b'|4|100010|',b'|4|140087|',b'|4|60074|']
a=addr
while a<end:
        ...
        if kernel32.ReadProcessMemory(h,ctypes.c_void_p(a),buf,sz,ctypes.byref(nr)):
            ...
            for t in targets:
                ...
                while True:
                    i=d.find(t,i)
                    if i==-1: break
                    # row starts at last space/tab before, ends at next space
                    s=d.rfind(b' ',0,i)
                    e=d.find(b' ',i)
                    if s==-1: s=max(0,i-60)
                    if e==-1: e=min(len(d),i+200)
                    row=d[s+1:e].decode('utf-8','ignore')
                    f=row.split('|')
                    print(t.decode(),'-> first field(row0)=',f[0] if f else '?',' name=',f[1] if len(f)>1 else '?',' skillcol=',f[14] if len(f)>14 else '?')
                    i+=1
    a+=sz
print('done')
